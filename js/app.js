...
(function () {
  ...
  // iOS Safari 는 키보드가 올라와도 레이아웃 뷰포트(100dvh)를 줄이지 않고, 포커스된 입력이 보이도록 시각 뷰포트를
  // 위로 밀어 올린다(헤더가 잘려 올라가고 키보드를 내리면 다시 튀어 내려옴). 목록 아래쪽도 키보드에 가려 스크롤로
  // 닿지 않는다. → 검색창에 키보드가 열려 있는 동안 body 를 '보이는 영역'(vv.offsetTop/height)에 고정해 그대로
  // 따라가게 한다(인앱 다이얼로그 fitViewport 와 같은 방식). iOS 가 얼마나 밀어 올리든 헤더·검색창·목록이 항상
  // 보이는 영역 안에 놓인다. scrollTo(0,0) 로 되돌리는 방식은 iOS 가 키보드 애니메이션 뒤 다시 밀어 올려 효과가 없었다.
  // 안드로이드는 viewport 메타(interactive-widget=resizes-content)로 뷰포트 자체가 줄어 innerHeight == vv.height
  // 이므로 여기서는 아무것도 하지 않는다. 오버레이(편집·설정 등)의 입력은 대상이 아님(고정 레이어 자체 스크롤 필요).
  (function () {
    var vv = window.visualViewport;
    if (!vv) return;
    var applied = false;
    ...
    function fit() {
      var kbOpen = (window.innerHeight - vv.height) > 120 && mainInputFocused();
      var b = document.body.style;
      if (kbOpen) {
        b.position = "fixed"; b.left = "0"; b.right = "0";
        b.top = vv.offsetTop + "px"; b.height = vv.height + "px";
        applied = true;
      } else if (applied) {
        b.position = ""; b.left = ""; b.right = ""; b.top = ""; b.height = "";
        applied = false;
        if (vv.offsetTop || window.scrollY) window.scrollTo(0, 0); // 키보드 내린 뒤 남은 밀림 원복
      }
    }
    vv.addEventListener("resize", fit);
    vv.addEventListener("scroll", fit);
    // iOS 키보드 애니메이션(약 250ms) 뒤 한 번 더 보정 — resize/scroll 이벤트 타이밍이 어긋나거나 누락돼도 맞춘다.
    searchInput.addEventListener("focus", function () { setTimeout(fit, 350); });
    searchInput.addEventListener("blur", function () { setTimeout(fit, 50); setTimeout(fit, 350); });
  })();
